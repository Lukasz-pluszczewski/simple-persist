...
function tmpDir(prefix = 'sp-e2e-') {
  const d = path.join(process.cwd(), '.tmp', prefix + Math.random().toString(36).slice(2));
...
export async function createTestServer(): Promise<TestServer> {
  ...
  app.use('/todos', persistCollection('todos-e2e', {
    baseDir,
    getTenant,
    validation: (item: any) => typeof item?.id === 'string' ? true : typeof item?.text === 'string'
  }));
